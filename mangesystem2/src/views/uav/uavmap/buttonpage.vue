...
<div>
  ...
  <el-tabs class="card-box" v-model="activeTab" type="border-card" v-show="showCard">
    <el-tab-pane label="任务设置" name="missionManage">
      <el-button type="primary" size="mini" @click="placeUserPoint">{{!editFtButton.placeUserPoint ?'用户位置':'完成'}}</el-button>
      <el-button type="primary" size="mini" @click="placeDepotPoint">{{!editFtButton.placeDepotPoint ?'起始位置':'完成'}}</el-button><br>
      <el-button type="primary" size="mini" @click="resetAll">重置</el-button>
      <el-button type="primary" size="mini" @click="pushAll">{{ !editFtButton.uploadStatus?'提交':'已提交' }}</el-button>
    </el-tab-pane>
